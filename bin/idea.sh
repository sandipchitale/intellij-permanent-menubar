...
if [ -z "$JRE" ] && [ -s "${CONFIG_HOME}/JetBrains/IntelliJIdea2024.3/idea.jdk" ]; then
  USER_JRE=$(cat "${CONFIG_HOME}/JetBrains/IntelliJIdea2024.3/idea.jdk")
...
# shellcheck disable=SC2034
IDE_CACHE_DIR="${XDG_CACHE_HOME:-${HOME}/.cache}/JetBrains/IntelliJIdea2024.3"
...
elif [ -r "${CONFIG_HOME}/JetBrains/IntelliJIdea2024.3/idea64.vmoptions" ]; then
  USER_VM_OPTIONS_FILE="${CONFIG_HOME}/JetBrains/IntelliJIdea2024.3/idea64.vmoptions"
fi
